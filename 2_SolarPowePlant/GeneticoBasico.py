...
def geneticoBasico(semillas=np.random.randint(-999, 999, size=5), nIndividuos=25,
...
def mutacionIntercambio(hijos, nMutacion):
    ...
    for j in range(len(hijos)):
        for i in range(int(nGenesMutados)):
            hijos[j] = fa.generarMovimientoAleatorio(hijos[j], 12)
...
def reemplazoTorneo(P, E, hijos, nReemplazo):
    ...
    for hijo in hijos:
        ...
        costesTorneo.append(s.evalua(hijo))
    individuosTorneo = np.array(individuosTorneo)
    costesTorneo = np.array(costesTorneo)
...
def estudioParametros(semillas, nIndividuos=25, nMejoresIndividuos=3, maxIteraciones=1000, nSeleccion=30, nMutacion=20,
                      nReemplazo=30):
    """
